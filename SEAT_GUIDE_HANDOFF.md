...
2. **座席タブ**: ゲート・スタンド・ブロック・列・番号＋方向矢印（8方向）を入力 → 相手の言語で全画面表示。下に日本語も小さく出る。
   - スタンドは `main / back / north / south` の4択（`STANDS`）。**エコパの実際の表記は未確認の仮置き**。
   - **キックオフ時刻ボタン**: `KICKOFFS = ["15:00", "19:30"]`。日程が変わったらここを書き換える。
3. **フレーズタブ**: 定型文 35 個、6カテゴリ（検索可）
   - `greet` あいさつ・チケット(4) / `seat` 座席案内(9) / `place` 施設の場所(8) / `match` 試合・駐車場(3) / `rule` お願い・ルール(6) / `sos` 緊急(5)
4. **お客様の返事タブ**: お客様が押すと日本語で大きく表示（はい／いいえ／わからない／トイレ／気分が悪い／連れとはぐれた 等 8個）。
5. **全画面表示の共通機能**: 上下反転（向かい合わせで見せる）、読み上げ（`speechSynthesis`、端末依存）。

### データ構造（`<script>` 内）
- `LANGS` … 言語一覧（`code, name, ja, speech, rtl`）
- `LABELS` … 座席カードの見出し等（言語コード → 文字列）
- `STANDS` … スタンド名の訳
...
- [ ] エコパのスタンド／ゲート／ブロックの実際の表記（公式座席表: https://www.ecopa.jp/facility/stadium/seat/ ）に合わせる。
